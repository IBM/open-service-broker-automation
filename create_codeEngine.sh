#!/usr/bin/env bash
###############################################################################
# SCRIPT NAME: create_codeEngine.sh
#
...
result=$(ibmcloud ce  app create --name osb-$lang-app --build-source $git_url --build-strategy dockerfile  --cpu 1 --memory 4G --ephemeral-storage 0.4G --min-scale 1 --port $port_num 2>&1)
exit_code=$?
echo $exit_code
if [ $exit_code -eq 0 ]
then
